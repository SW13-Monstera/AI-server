...
class KeywordPredictRunnable(bentoml.Runnable):
    ...
    def synchronize_keywords(self, input_data: KeywordInferenceData) -> None:
        ...
        remain_keywords = []
        input_keyword_dict = {
            input_keyword.id: input_keyword.content
            for input_keyword in input_data.keywords
        }
        ...
        if is_keyword_changed:
            for new_keyword_id, new_keyword_content in input_keyword_dict.items():
                remain_keywords.append(
                    Keyword(id=new_keyword_id, content=new_keyword_content)
                )
            self.problem_dict[problem_id].keywords = remain_keywords
            new_embedded_keywords = self.model.encode(
                [keyword.content for keyword in remain_keywords]
            )
            self.problem_dict[problem_id].embedded_keywords = new_embedded_keywords

    @bentoml.Runnable.method(batchable=False)
    def is_correct_keyword(
        self, input_data: KeywordInferenceData
    ) -> KeywordInferenceResponse:
        # 키워드 변경 감지도 추가 해야함
        ...
        if input_data.problem_id not in self.problem_dict:
            self.problem_dict[input_data.problem_id] = Problem(
                ...
                embedded_keywords=self.model.encode(
                    [keyword.content for keyword in input_data.keywords]
                ),
            )
        ...
        for k in range(len(split_answer) - self.word_concat_size + 1):
            tokenized_answer.append(
                " ".join(split_answer[k : k + self.word_concat_size])
            )
        ...
        similarity_scores = cosine_similarity(
            problem.embedded_keywords, tokenized_answer_embedding
        )
        predicts = []
        ...
        return KeywordInferenceResponse(
            problem_id=input_data.problem_id, correct_keywords=predicts
        )
